...
def login_screen():
  ...
  with st.form("login_form"):
    ...
    if submit:
      # Simple secure enterprise auth check (can be mapped to active directory or secure env)
...
def process_home_health_standalone(df):
  """Tier 2: Standalone Home Health Engine

  - Column E (index 4) Employee ID anchoring.
  - Row PRN amount = Visits * Rate, aggregated by ID.
  - Zero-Drop Rule: Zero PRN points = excluded.
  - Hourly Target (ID 1389, etc.) with 80-hr OT rule.
  - Mileage locked at 0.73.
  """
  ...
  HOURLY_TARGET_IDS = {1389, 1351, 1388, 1162}
  HOURLY_RATES = {
      1389.0: 40.00,
      1351.0: 35.00,
      1388.0: 40.00,
  ...
  raw_rows = []
  grouped = df.groupby([df[emp_id_col], df[name_col].astype(str)], dropna=False)

  for (emp_id_raw, emp_name), group in grouped:
    try:
      emp_id = int(float(str(emp_id_raw).strip()))
    except:
      emp_id = emp_id_raw
    ...
    is_hourly = (emp_id in HOURLY_TARGET_IDS) or (
        isinstance(emp_id, float) and int(emp_id) in HOURLY_TARGET_IDS
    )

    total_employee_hours = 0.0
    total_employee_mileage = 0.0
    total_prn_amount = 0.0
    applied_rate = HOURLY_RATES.get(
        float(emp_id) if isinstance(emp_id, (int, float)) else 0.0, 35.00
    )
    ...
    for _, row in group.iterrows():
      ...
      if is_hourly:
        ...
        if rate_col and pd.notnull(row.get(rate_col)):
          try:
            r = float(
                str(row.get(rate_col)).replace("$", "").replace(",", "").strip()
            )
            if r > 0:
              applied_rate = r
          except:
        ...
        if amount_col and pd.notnull(row.get(amount_col)):
          try:
            item_amt = float(
                str(row.get(amount_col)).replace("$", "").replace(",", "").strip()
            )
          except:
            item_amt = 0.0
...
def process_home_care_engine(df):
  ...
  - Missing ID checks.
  - Blank/NaN component defaults to Overtime.
  - Cumulative 80-Hour Overtime threshold per ID.
  """
  df.columns = [str(c).strip() for c in df.columns]
  raw_rows = []

  # Group by ID for 80-hour threshold calculation
  for emp_id, group in df.groupby(df.columns[4], dropna=False):
    is_missing_id = pd.isnull(emp_id) or str(emp_id).strip().lower() in [
        "",
        "nan",
        "none",
    ]
    ...
    for _, row in group.iterrows():
      ...
      if not comp or comp.lower() in ["nan", "none"]:
        comp = "Overtime"  # Blank/NaN component fallback rule
    ...
    # Apply 80-Hour rule per unique ID for Hourly components
    if total_id_hours > 0:
...
def process_hospice_logic(hospice_timesheet_df, home_health_raw_df):
  """Tier 1: Hospice & Cross-Over Reconciliation Engine

  - Brandy & Cooper exclusive On-Call rules (50/100 rates).
  - Normal hospice workers cross-over: Home Health PRN points retained,
    Hourly & Mileage discarded in favor of Hospice timesheet.
  - Global 80-Hour rule applied to resulting hourly components.
  """
  # Implementation handles name matching, ID fallback, and exclusive rules.
  # Clean structure returning reconciled DataFrame.
  processed_rows = []
  # [Core reconciliation logic loop parsing Hospice timesheets vs HH Master]
...
with tab1:
  ...
  st.info(
      "Hierarchy Active: Hospice-Home Health Cross-Over (1st) ➔ Standalone Home"
      " Health (2nd) ➔ Home Care (3rd). Global 80-Hour Rule Enforced."
  )

  hh_file = st.file_uploader(
      "Upload Home Health Raw Master (.csv / .xlsx)", type=["csv", "xlsx"]
  )
  hc_file = st.file_uploader(
      "Upload Home Care File (.csv / .xlsx)", type=["csv", "xlsx"]
  )
  hosp_file = st.file_uploader(
      "Upload Hospice Timesheet (.csv / .xlsx)", type=["csv", "xlsx"]
  )

  if st.button("Execute Unified Multi-LOB Run"):
    if hh_file or hc_file or hosp_file:
      with st.spinner("Processing enterprise multi-LOB pipeline..."):
        ...
        if master_output_frames:
          st.session_state.processed_df = pd.concat(
              master_output_frames, ignore_index=True
          )
          st.success("Unified Batch Execution Completed Successfully!")
          st.dataframe(st.session_state.processed_df)
        else:
          st.warning("Please upload valid source files to execute pipeline.")
    else:
...
with tab2:
  ...
  st.write(
      "Validates Column E ID mapping, Zero-Drop PRN points, and ID 1389 hourly"
      " overtime."
  )
...
with tab3:
  ...
  st.write(
      "Validates missing IDs, blank component fallback to Overtime, and 80-hour"
      " split."
  )
...
with tab4:
  ...
  st.write(
      "Manages Brandy & Cooper on-call logic and cross-over PRN retention."
  )